...
describe('MagnetAcademy', function () {
  ...
  describe('MagnetAcademy deployment', function () {
    ...
    it('Should have rector as role administrator', async function () {
      const ADMIN = await magnetAcademy.ADMIN();
      const RECTOR = await magnetAcademy.RECTOR();
      expect(await magnetAcademy.getRoleAdmin(ADMIN)).to.equal(RECTOR);
    });
  ...
  describe('Administrators system management', function () {
    let ADMIN, RECTOR;
    beforeEach(async function () {
      ADMIN = await magnetAcademy.ADMIN();
      RECTOR = await magnetAcademy.RECTOR();
      // academyAdmin1 is an admin for our tests
      await magnetAcademy.connect(rector).grantRole(await magnetAcademy.ADMIN(), academyAdmin1.address);
    });

    it('A rector should be able to nominate an admin', async function () {
      await magnetAcademy.connect(rector).grantRole(ADMIN, academyAdmin2.address);
    ...
    it('Should emit RoleGranted event when admin is added', async function () {
      await expect(magnetAcademy.connect(rector).grantRole(ADMIN, academyAdmin2.address))
        .to.emit(magnetAcademy, 'RoleGranted')
        .withArgs(ADMIN, academyAdmin2.address, rector.address);
    });
    ...
    it('A rector should be able to revoke an admin', async function () {
      await magnetAcademy.connect(rector).grantRole(ADMIN, academyAdmin1.address);
      await magnetAcademy.connect(rector).revokeRole(ADMIN, academyAdmin1.address);
    ...
    it('Should emit RoleRevoked event when admin is revoked', async function () {
      await magnetAcademy.connect(rector).grantRole(ADMIN, academyAdmin1.address);
      await expect(magnetAcademy.connect(rector).revokeRole(ADMIN, academyAdmin1.address))
        .to.emit(magnetAcademy, 'RoleRevoked')
        .withArgs(ADMIN, academyAdmin1.address, rector.address);
    });
  ...
  describe('School creation', function () {
    let school1Address, tx, ADMIN, RECTOR;
    beforeEach(async function () {
      ADMIN = await magnetAcademy.ADMIN();
      RECTOR = await magnetAcademy.RECTOR();
    ...
    it('Should revert if not created by admin', async function () {
      await expect(magnetAcademy.connect(lambdaUser).createSchool(school2Name, director2.address)).to.be.revertedWith(
        'AccessControl: account 0x15d34aaf54267db7d7c367839aaf71a00a2c6a65 is missing role 0xdf8b4c520ffe197c5343c6f5aec59570151ef9a492f2c624fd45ddde6135ec42'
      );
    });
  ...
  describe('School destruction', function () {
    let school1Address, ADMIN, RECTOR;
    beforeEach(async function () {
      ADMIN = await magnetAcademy.ADMIN();
      RECTOR = await magnetAcademy.RECTOR();
      // compute school1Address before deployment
  ...
  describe('School Directors changes', function () {
    ...
    it('Should revert if directors are not changed by admin', async function () {
      ...
      ).to.be.revertedWith('AccessControl: account 0x15d34aaf54267db7d7c367839aaf71a00a2c6a65 is missing role 0xdf8b4c520ffe197c5343c6f5aec59570151ef9a492f2c624fd45ddde6135ec42');
    });
